...
{
	Vector2D C = Vector2D( B.x - A.x, B.y - A.y );
	return sqrt( C.x * C.x + C.y * C.y );
}

float Dist_2D( float x1, float y1, float x2, float y2 )
{
	return Dist_2D( Vector2D( x1, y1 ), Vector2D( x2, y2 ) );
}

float Dist_3D( Vector3D A, Vector3D B )
{															 
	Vector3D C = Vector3D( B.x - A.x, B.y - A.y, B.z - A.z );
	return sqrt( C.x * C.x + C.y * C.y + C.z * C.z );
}

float Dist_3D( float x1, float y1, float z1, float x2, float y2, float z2 )
{
	return Dist_3D( Vector3D( x1, y1, z1 ), Vector3D( x2, y2, z2 ) );
}
